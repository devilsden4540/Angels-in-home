                ...
                <div class="col-12">
                    <div class="row justify-content-center">
                        <div class="col-md-7 text-center hero-text">
                            <h1 data-aos="fade-up" data-aos-delay="">Customer registration</h1>
...
if (isset($_POST['submit'])) {
    ...
    if ($password == $cPassword) {
        $qryChe = "SELECT COUNT(*) FROM `login` WHERE `uname`='$email'";
        // echo $qryChe;
        $resChe = mysqli_query($con, $qryChe);
        // $count = mysqli_num_rows($resChe);
        $rowChe = mysqli_fetch_array($resChe);
        $count = $rowChe[0];
        if ($count == 0) {
            $qry = "INSERT INTO `customers` (`name`,`email`,`phone`,`address`,`district`) VALUES ('$name','$email','$phone','$address','$district')";
            $qryLog = "INSERT INTO `login` (`uid`,`uname`,`password`,`utype`,`status`) VALUES ((SELECT MAX(`cid`) FROM `customers`), '$email','$password','Customer','Active')";
